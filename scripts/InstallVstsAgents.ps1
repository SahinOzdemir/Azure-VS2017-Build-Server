...
function PrepMachineForAutologon () {
    ...
    if ($username.Split("\").Count -eq 2)
    {
      ...
      $usrName = $username.Split('\')[1]
    }
    else
    {
      $domain = $Env:ComputerName
      $usrName = $username
      Write-Verbose "Username constructed to use for creating a PSSession: $domain\\$usrName"
    }
   
    $credentials = New-Object System.Management.Automation.PSCredential("$domain\\$usrName", $pass)
...
{
  $pass = ConvertTo-SecureString $password -AsPlainText -Force
  New-LocalUser $username -Password $pass -FullName "Build agent service account" -Description "N/A" -AccountNeverExpires 
  Add-LocalGroupMember -Group "Users" -Member $username
}

Write-Verbose "Entering InstallVSOAgent.ps1" -verbose
Write-Verbose "Create service account"
CreateUser
...
do
{
  try
  {
    ...
    $latestReleaseDownloadUrl = "https://vstsagentpackage.azureedge.net/agent/2.134.2/vsts-agent-win-x64-2.134.2.zip"
...
#Loop and create the number of agents
For ($i=1; $i -le $NumberOfAgents; $i++) 
{

    # Construct the agent folder under the main (hardcoded) F: drive.
    #$guid = [GUID]::NewGuid().GUID
    $agentInstallationPath = Join-Path "F:\Agents" ($AgentName+$i)
    ...
    if ($runAsAutoLogon -ieq "true")
    {
      ...
      .\config.cmd --unattended --url $serverUrl --auth PAT --token $PersonalAccessToken --pool $PoolName --agent ($AgentName+$i) --runAsAutoLogon --overwriteAutoLogon --windowslogonaccount $username --windowslogonpassword $password
    }
    else 
    {
      # Setup the agent as a service
      .\config.cmd --unattended --url $serverUrl --auth PAT --token $PersonalAccessToken --pool $PoolName --agent ($AgentName+$i) --runasservice
    }
